...
export class ParentManagementComponent implements OnInit {
  ...
  parents: Parent[];
  displayedColumns = ['id','name','email','phoneno','address', 'action'];
  dataSource;
  position = 'above';
  ...
  getParent(): void {
    this.parentManagementService.get()
      .subscribe(parents => {this.parents = parents,this.bindData()});
  ...
  onEdit(event):void{
    this.router.navigate(['/edit', { id: event,type:'updateParent'}]);
  }
  onAdd():void{
    this.router.navigate(['/edit', {type:'newParent'}]);
  }
