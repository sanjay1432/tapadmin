...
export class TeacherManagementComponent implements OnInit {
  ...
  teachers: Teacher[];
  displayedColumns = ['id', 'name','email','phoneno','report','action'];
  dataSource;
  position = 'above';
  ...
  getTeacher(): void {
    this.teacherManagementService.get()
      .subscribe(teachers => {this.teachers = teachers, this.bindData();});
  ...
  onEdit(event):void{
    this.router.navigate(['/edit', { id: event,type:'updateTeacher'}]);
  }
  onAdd():void{
    this.router.navigate(['/edit', {type:'newTeacher'}]);
  }
  onRemove(event):void{
